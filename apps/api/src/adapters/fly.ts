...
export const flyAdapter: ProviderAdapter = {
  async run(params, apiKey) {
    if (!apiKey || apiKey === 'PLACEHOLDER' || apiKey.startsWith('sk-test')) {
      throw new ProviderError('Fly API key is not configured on this gateway or requires a valid BYOK key', true);
    }

    try {
      // Wire API call for Fly
      const r = await axios.post(
        'https://api.fly.com/v1/execute',
        { ...params },
      ...
      throw new ProviderError(`Fly API Error: ${err.message}`, isQuota);
    }
  },
};
