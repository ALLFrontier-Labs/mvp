...
export const perplexityAdapter: ProviderAdapter = {
  async run(params, apiKey) {
    if (!apiKey || apiKey === 'PLACEHOLDER' || apiKey.startsWith('sk-test')) {
      throw new ProviderError('Perplexity API key is not configured on this gateway or requires a valid BYOK key', true);
    }

    try {
      // Wire API call for Perplexity
      const r = await axios.post(
        'https://api.perplexity.com/v1/execute',
        { ...params },
