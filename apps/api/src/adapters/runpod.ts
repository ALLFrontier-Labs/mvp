...
export const runpodAdapter: ProviderAdapter = {
  async run(params, apiKey) {
    if (!apiKey || apiKey === 'PLACEHOLDER' || apiKey.startsWith('sk-test')) {
      throw new ProviderError('Runpod API key is not configured on this gateway or requires a valid BYOK key', true);
    }

    try {
      // Wire API call for Runpod
      const r = await axios.post(
        'https://api.runpod.com/v1/execute',
        { ...params },
