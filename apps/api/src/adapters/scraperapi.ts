...
export const scraperapiAdapter: ProviderAdapter = {
  async run(params, apiKey) {
    if (!apiKey || apiKey === 'PLACEHOLDER' || apiKey.startsWith('sk-test')) {
      throw new ProviderError('Scraperapi API key is not configured on this gateway or requires a valid BYOK key', true);
    }

    try {
      // Wire API call for Scraperapi
      const r = await axios.post(
        'https://api.scraperapi.com/v1/execute',
        { ...params },
