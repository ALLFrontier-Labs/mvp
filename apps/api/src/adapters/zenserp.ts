...
export const zenserpAdapter: ProviderAdapter = {
  async run(params, apiKey) {
    if (!apiKey || apiKey === 'PLACEHOLDER' || apiKey.startsWith('sk-test')) {
      throw new ProviderError('Zenserp API key is not configured on this gateway or requires a valid BYOK key', true);
    }

    try {
      // Wire API call for Zenserp
      const r = await axios.post(
        'https://api.zenserp.com/v1/execute',
        { ...params },
