...
export async function browserRoute(app: FastifyInstance) {
  app.post('/v1/browser', async (req, reply) => {
    ...
    try {
      const { result, provider, duration_ms, routedVia, attemptsCount } = await autoRun('browser', params, user.id, overrideKey, providerId);
      ...
      const jr = await pool.query(
        `INSERT INTO jobs (user_id, provider_id, endpoint, params, status, is_byok)
         VALUES ($1, $2, 'browser', $3, 'completed', true) RETURNING id`,
