...
export async function browserRoute(app: FastifyInstance) {
  app.post('/v1/browser', async (req, reply) => {
    ...
    const targetProvider = providerId === 'auto' ? 'browserbase' : providerId;
    ...
    try {
      const { result, provider, charge: baseCharge, duration_ms, routedVia, attemptsCount } = await autoRun('browser', params, user.id, overrideKey, providerId);
      ...
      const jr = await pool.query(
        `INSERT INTO jobs (user_id, provider_id, endpoint, params, status, cost_usd, is_byok)
         VALUES ($1, $2, 'browser', $3, 'completed', $4, true) RETURNING id`,
      ...
      if (finalCharge > 0) {
        try {
          await debitLedger(user.id, finalCharge, provider.id, jobId, `${provider.name} browser session gateway fee`);
        } catch (err: any) {
      ...
      return reply.code(status).send({
        error: (err as any).code || 'browser_failed',
        message: err.message,
      });
    }
  });
