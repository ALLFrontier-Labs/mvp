...
export async function searchRoute(app: FastifyInstance) {
  app.post('/v1/search', async (req, reply) => {
    ...
    try {
      const { result, provider, charge, duration_ms, routedVia, attemptsCount } = await autoRun('search', params, user.id, overrideKey);
      ...
      const jr = await pool.query(
        `INSERT INTO jobs (user_id, provider_id, endpoint, params, status, cost_usd, is_byok)
         VALUES ($1, $2, 'search', $3, 'completed', $4, true) RETURNING id`,
      ...
      if (charge > 0) {
        try {
          await debitLedger(user.id, charge, provider.id, jobId, `${provider.name} search gateway fee — ${params.query}`);
        } catch (err: any) {
