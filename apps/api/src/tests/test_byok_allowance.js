...
async function runTest() {
  // Create temp test user with 99 existing requests
  const email = `byok_test_${Date.now()}@litedaemon.internal`;
  const userRes = await pool.query(
    `INSERT INTO users (email, balance_usd, monthly_call_count, byok_requests_this_month, billing_period_start)
     VALUES ($1, 0.00000000, 99, 99, NOW()) RETURNING id`,
    [email]
  );
  const userId = userRes.rows[0].id;
  console.log(`📌 Created test user ${email} (ID: ${userId}) with 99 existing requests & $0.00 balance`);

  try {
    // ── Step 1: Request #1 (100th call - Free Allowance) ──────────────────────
    console.log('\n🔹 Step 1: Firing Request #1 (100th call)...');
    const eval1 = await preCheckAndEvaluateByok(userId, 'tavily');
    assert.strictEqual(eval1.allowed, true, '100th call should be ALLOWED');
    assert.strictEqual(eval1.isFreeCall, true, '100th call should be marked as FREE');
    assert.strictEqual(eval1.charge, 0, '100th call charge should be $0.00');
    assert.strictEqual(eval1.nextRequestCount, 100, 'Next request count should be 100');
    console.log('  ✅ [PASS] 100th call ALLOWED with $0.00 charge (Free Monthly Allowance)');

    // ── Step 2: Request #2 (101st call - Overage on $0 Balance) ──────────────
    console.log('\n🔹 Step 2: Firing Request #2 (101st call) on $0.00 balance...');
    const eval2 = await preCheckAndEvaluateByok(userId, 'tavily');
    assert.strictEqual(eval2.allowed, false, '101st call on $0 balance should be REJECTED');
    ...
    await pool.query('UPDATE users SET balance_usd = 5.00000000, credit_balance = 5.00000000 WHERE id = $1', [userId]);
    console.log('  ✅ [PASS] Wallet topped up to $5.00000000 USD');

    // ── Step 4: Request #3 (101st call Retry on $5.00 Balance) ───────────────
    console.log('\n🔹 Step 4: Retrying Request #3 (101st call) with $5.00 balance...');
    const eval3 = await preCheckAndEvaluateByok(userId, 'tavily');
    assert.strictEqual(eval3.allowed, true, '101st call with $5.00 balance should be ALLOWED');
    assert.strictEqual(eval3.isFreeCall, false, '101st call is not free');
    assert.strictEqual(eval3.charge, 0.00005, '101st call 5% BYOK fee should be $0.00005');
    ...
    console.log(`  ✅ [PASS] 101st call ALLOWED! Debited $0.00005 fee. Remaining wallet balance = $${finalBalance.toFixed(5)} USD`);
